...
public class RunSimulator
{
    ...
    private static void EnsureModelDbInitialized()
    {
        ...
        // Newer sts2.dll versions gate ReflectionHelper.ModTypes behind ModManager init.
        // Harmony-patch get_ModTypes to return empty array (no mods in headless).
        try
        {
            var harmony = new Harmony("sts2headless.modtypes");
            var reflHelperType = typeof(MegaCrit.Sts2.Core.Models.ModelDb).Assembly
                .GetType("MegaCrit.Sts2.Core.Helpers.ReflectionHelper");
            var modTypesProp = reflHelperType?.GetProperty("ModTypes",
                System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.Public);
            var getter = modTypesProp?.GetGetMethod();
            if (getter != null)
            {
                var prefix = typeof(ModTypesPatch).GetMethod(nameof(ModTypesPatch.Prefix),
                    System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.Public);
                harmony.Patch(getter, new HarmonyMethod(prefix));
                Console.Error.WriteLine("[INFO] Patched ReflectionHelper.ModTypes → empty (no mods)");
            }
            else
            {
                Console.Error.WriteLine("[WARN] ReflectionHelper.ModTypes getter not found");
            }
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"[WARN] ModTypes patch: {ex.Message}");
        }
    ...
    internal static class ModTypesPatch
    {
        public static bool Prefix(ref Type[] __result)
        {
